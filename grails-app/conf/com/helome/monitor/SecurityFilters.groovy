...
class SecurityFilters {
    ...
    static authenticatedActions = [
            [controller: '*', action: '*', roles: ['ROLE_ADMIN']],
            [controller: '*', action: 'index, list, edit', roles: ['ROLE_USER']]
    ]
...
//            [controller: 'monitor', action: 'index'],
    ...
    def onNotAuthenticated(subject, filter) {
        filter.redirect(controller: "auth", action: "login")
    }
...
//        }
        all(controller: '*', action: '*') {
            before = {
				...
				if(controllerName.equals("auth") && actionName.equals("login") && SecurityUtils.subject.authenticated){
                ...
                if (needsAuth) {
                    ...
                    def authRoles = authenticatedActions.find {
                        ((it.controller == '*') || (it.controller == controllerName)) &&
                                (it.action.contains(actionName) || (it.action == '*'))
                    }
